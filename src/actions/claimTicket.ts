...
export const registerVerifyBars = async () => {
    ...
    //CLAIM TICKET STAFF MESSAGE
    opendiscord.verifybars.add(new api.ODVerifyBar("opendiscord:claim-ticket-staff-message",opendiscord.builders.messages.getSafe("opendiscord:verifybar-ticket-message"),!generalConfig.data.system.disableVerifyBars))
    opendiscord.verifybars.get("opendiscord:claim-ticket-staff-message")!.success.add([
        new api.ODWorker("opendiscord:permissions",1,async (instance,params,source,cancel) => {
            //reuse same permission logic
            const permissionMode = generalConfig.data.system.permissions.claim
            if (permissionMode == "none"){
                instance.reply(await opendiscord.builders.messages.getSafe("opendiscord:error-no-permissions").build("button",{guild:instance.guild,channel:instance.channel,user:instance.user,permissions:[]}))
                return cancel()
        ...
        new api.ODWorker("opendiscord:claim-ticket",0,async (instance,params,source,cancel) => {
            ...
            if (params.data == "reason"){
                instance.reply(await opendiscord.builders.messages.getSafe("opendiscord:error").build("button",{guild,channel,user,layout:"simple",error:"Reasons are disabled in staff-controls buttons. Use `/claim reason:` instead.",customTitle:"No Permissions"}))
                return cancel()
            }

            await instance.defer("update",false)
            await opendiscord.actions.get("opendiscord:claim-ticket").run("ticket-message",{guild,channel:(channel.isThread() ? await opendiscord.client.fetchGuildTextChannel(guild,ticket.id.value) : channel) as any,user,ticket,reason:null,sendMessage:true})
    ...
    opendiscord.verifybars.get("opendiscord:claim-ticket-staff-message")!.failure.add([
